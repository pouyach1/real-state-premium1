...
async function logout(req, res, next) { try { await authService.logout(req.cookies.refreshToken); res.clearCookie('refreshToken', refreshCookie); return success(res, null, 204); } catch (error) { return next(error); } }
